...
pub fn clear_for_search(position: &mut Board, info: &SearchInfo, hash_table: &mut HashTable) {
    for i in 0..13 {
...
pub fn search_position(position: &mut Board, info: Arc<SearchInfo>, hash_table: &mut HashTable) {
    let mut best_move = NO_MOVE;
