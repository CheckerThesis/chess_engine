...
// null move is switching to the opponent and giving them a free move
...
pub fn alpha_beta(alpha: &mut i32, beta: &mut i32, mut depth: i32, position: &mut Board, info: &mut SearchInfo, do_null: bool) -> i32 {
    ...
    // because if one check, likely a sequence of checks into mate, with this, it will see that and respond accordingly
    if in_check { depth += 1; }

    let mut score: i32 = -INFINITE;
    ...
    let mut best_score: i32 = -INFINITE;
    ...
    for move_number in 0..move_list.count {
        ...
        if score > best_score {
            ...
            if score > internal_alpha {
                ...
                // history heuristic
                if move_list.moves[move_number].el_move & MOVE_FLAG_CAPTURE == 0 {
                    // let clamped_bonus = depth.clamp(-90000, 90000);
                    // position.search_history[position.pieces[from_square(best_move) as usize] as usize][to_square(best_move) as usize] +=
                    // clamped_bonus as u32 - position.search_history[position.pieces[from_square(best_move) as usize] as usize][to_square(best_move) as usize] as u32 *
                    // clamped_bonus.abs() as u32 / 90000;
...
pub fn search_position(position: &mut Board, info: &mut SearchInfo) {
    ...
    for current_depth in 0..info.depth {
        best_score = alpha_beta(&mut -INFINITE, &mut INFINITE, current_depth + 1, position, info, true);
