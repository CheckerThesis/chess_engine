...
use crate::{board::Board, data::{FILE_CHAR, RANK_CHAR}, defs::{Castling::*, Pieces::*, Squares::*, BOARD_SQUARE_NUMBER, DEBUG, ENGINE_OPTIONS, FILES_BOARD, NO_MOVE, RANKS_BOARD, WHITE}, io::parse_move};
...
pub fn has_pawn_for_capture(position: &mut Board) -> bool {
    ...
    if position.side == WHITE as u8 {
        target_piece = WhitePawn as u8;
    } else {
        target_piece = BlackPawn as u8;
    }

    if position.en_passent != NoSq as u8{
        if position.side == WHITE as u8 {
            square_with_pawn = position.en_passent - 10;
        } else {
            square_with_pawn = position.en_passent + 10;
        }
        if position.pieces[square_with_pawn as usize - 1] == target_piece {
            return true
        } else if position.pieces[square_with_pawn as usize + 1] == target_piece {
            return true
        }
    }
...
pub fn polykey_from_board(position: &mut Board) -> u64 {
    ...
    for square in 0..BOARD_SQUARE_NUMBER {
        ...
        if piece != NoSq as u8 && piece != Empty as u8 && piece != OffBoard as u8 {
            if DEBUG && piece > BlackKing as u8 || piece < WhitePawn as u8 { eprintln!("{}", "polykey_from_board: [piece] not valid".red()); }

            let poly_piece = POLY_KIND_OF_PIECE[piece as usize] as usize;
            let rank = RANKS_BOARD[square] as usize;
            let file = FILES_BOARD[square] as usize;

            final_key ^= RANDOM_POLY[(64 * poly_piece) + (8 * rank) + file];
        }
    }

    // castling
    if position.castle_permission & WhiteKingCastle as u8 != 0 { final_key ^= RANDOM_POLY[768 + 0] }
    if position.castle_permission & WhiteQueenCastle as u8 != 0 { final_key ^= RANDOM_POLY[768 + 1] }
    if position.castle_permission & BlackKingCastle as u8 != 0 { final_key ^= RANDOM_POLY[768 + 2] }
    if position.castle_permission & BlackQueenCastle as u8 != 0 { final_key ^= RANDOM_POLY[768 + 3] }

    // enpassant
    if has_pawn_for_capture(position) {
        let file = FILES_BOARD[position.en_passent as usize];
        final_key ^= RANDOM_POLY[772 + file as usize];
    }

    if position.side == WHITE as u8 { final_key ^= RANDOM_POLY[780]; }
...
pub fn convert_poly_move(poly_move: u16, position: &mut Board) -> u32 {
    ...
    let to_rank = (poly_move >> 3) & 7;
    let promotion_piece = (poly_move >> 12) & 7;

    let mut char_vec: Vec<char> = vec![
        FILE_CHAR.chars().nth(from_file as usize).unwrap_or(' '),
        RANK_CHAR.chars().nth(from_rank as usize).unwrap_or(' '),
        FILE_CHAR.chars().nth(to_file as usize).unwrap_or(' '),
        RANK_CHAR.chars().nth(to_rank as usize).unwrap_or(' ')
    ];
...
pub fn get_book_move(position: &mut Board) -> u32 {
    ...
    for i in 0..POLY_BOOK.len() {
        if poly_key == POLY_BOOK[i].key.to_be() {
            let the_move = POLY_BOOK[i].the_move.to_be();
            let temp_move = convert_poly_move(the_move, position);

            if temp_move != NO_MOVE {
                book_moves[count] = temp_move as u32;
                count += 1;
                if count > 32 { break; }
            }
        }
